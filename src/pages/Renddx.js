...
import { ArrowRight, Clock, Users, Zap, Target, CreditCard, Loader2, AlertTriangle, Laptop, Rocket, X, Monitor } from "lucide-react";
...
const Renddx = () => {
    ...
    const [accessDenyReason, setAccessDenyReason] = useState('Voce precisa assistir pelo menos 60% do video em /renddx para liberar essa pagina.');
    ...
    useEffect(() => {
        ...
        const emailFromStorage = (() => { try {
            return localStorage.getItem('renddx-desconto:email') || '';
        }
        catch {
            return '';
        } })();
        ...
        supabase.functions.invoke('rendaextra-desconto-access', {
            ...
            try {
                ...
                if (data.name)
                    localStorage.setItem('renddx-desconto:name', data.name);
            }
            catch { }
