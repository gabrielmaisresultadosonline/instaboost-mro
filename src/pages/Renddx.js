...
import { ArrowRight, Clock, Users, Zap, Target, CreditCard, Loader2, AlertTriangle, Laptop, Rocket, X, Monitor, Check, MousePointer2 } from "lucide-react";
...
const Renddx = () => {
    ...
    return (<div className="min-h-screen bg-black text-white overflow-x-hidden">
      ...
      {showCheckoutModal && (<div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/80 p-4">
          <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-6 max-w-lg w-full relative max-h-[90vh] overflow-y-auto custom-scrollbar">
            ...
            <form onSubmit={handleCheckout} className="space-y-6">
              ...
              <div className="space-y-4 pt-4 border-t border-zinc-800">
                ...
                {loadingProducts ? (<div className="flex justify-center py-4">
                    <Loader2 className="w-6 h-6 animate-spin text-zinc-700"/>
                  </div>) : products.map((prod) => (<div key={prod.id} onClick={() => toggleBump(prod.slug)} className={`group relative overflow-hidden rounded-xl border-2 transition-all cursor-pointer p-3 ${selectedBumps.includes(prod.slug)
                    ...
                    <div className="flex gap-3 items-center">
                      <div className="flex-1 space-y-1">
                        <div className="flex justify-between items-start">
                          ...
                          <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-colors relative ${selectedBumps.includes(prod.slug) ? 'bg-green-500 border-green-500' : 'border-zinc-700'}`}>
                            {selectedBumps.includes(prod.slug) ? (<Check className="w-3 h-3 text-black font-bold"/>) : (<div className="absolute -left-6 top-1/2 -translate-y-1/2">
