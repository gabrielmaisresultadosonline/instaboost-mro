...
const AudiobooksPreCheckout = () => {
  const navigate = useNavigate();
  const baseAmount = 37;
  const productSlug = "audiibooks";
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      localStorage.setItem('audiobooks_checkout_amount', totalAmount.toString());
      ...
      const { data, error } = await supabase.functions.invoke("hub-api", {
        body: { 
          ...
          name: name.trim(),
          whatsapp: phone.replace(/\D/g, "").trim(),
          orderBumps: {
            lifetime: selectedBumps.includes('lifetime-bump'),
            analysis: selectedBumps.includes('analysis-bump')
          },
          // A Edge Function hub-api já lida com os bumps específicos de áudio, 
          // mas vamos garantir que o valor total seja passado se necessário.
          amount: totalAmount
        }
      });
      ...
      trackInitiateCheckout('Audiobooks Checkout', totalAmount);
      ...
      <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-8 items-start pb-20">
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div className="space-y-2">
            <h1 className="text-3xl font-black bg-gradient-to-r from-yellow-400 to-orange-500 bg-clip-text text-transparent italic uppercase">
              Finalize seu Acesso
            </h1>
            <p className="text-zinc-400 font-bold">O SEGREDO PARA VENDER MAIS!</p>
          </div>

          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-bold text-zinc-500 ml-1">NOME COMPLETO</label>
              <Input 
                value={name}
        ...
        <div className="space-y-6">
          <div className="bg-zinc-900/50 border border-zinc-800 rounded-3xl p-6 md:p-8 space-y-6">
            <div className="flex items-center justify-between border-b border-zinc-800 pb-4">
              <div>
                <h2 className="font-black text-xl italic uppercase">Resumo</h2>
                <p className="text-zinc-500 text-sm font-bold uppercase">Audiobooks + Ebooks</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-zinc-500 uppercase font-black tracking-tighter">Total</p>
                <p className="font-black text-yellow-400 text-2xl italic">R$ {totalAmount.toFixed(2).replace('.', ',')}</p>
              </div>
            </div>

            <div className="space-y-3 py-2 border-b border-zinc-800/50">
              <div className="flex justify-between items-center">
                <span className="text-sm font-bold text-zinc-300">O Segredo para Vender Mais</span>
                <span className="text-sm font-black text-white flex items-center gap-2 italic">
                  R$ {baseAmount.toFixed(2).replace('.', ',')}
              ...
              {selectedBumps.map(slug => {
                ...
                return (
                  <div key={slug} className="flex justify-between items-center animate-in fade-in slide-in-from-right-2 duration-300">
                    <span className="text-xs text-zinc-400 font-bold flex items-center gap-1 uppercase">
                      <Check className="w-3 h-3 text-yellow-400" /> {prod.title}
                    </span>
                    <span className="text-xs font-bold text-zinc-300 flex items-center gap-2 italic">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-black text-zinc-200 uppercase tracking-widest flex items-center gap-2 italic">
                ...
                >
                  ...
                  <div className="flex gap-4 items-center">
                    <div className="w-16 h-16 rounded-xl overflow-hidden bg-zinc-800 flex-shrink-0">
                      <img 
                        src={prod.thumb_url || prod.image || "https://adljdeekwifwcdcgbpit.supabase.co/storage/v1/object/public/assets/ebooks/covers/1786318391838-capa_48_leis.png"} 
                        alt={prod.title} 
                    ...
                    <div className="flex-1 space-y-1">
                      <div className="flex justify-between items-start">
                        <h3 className="font-bold text-sm leading-tight italic uppercase">{prod.title}</h3>
                      ...
                      <p className="text-[11px] text-zinc-400 line-clamp-2 font-medium">{prod.description}</p>
                      <div className="flex items-center gap-2 pt-1">
                        <p className="text-yellow-400 font-black text-sm italic">
                          + R$ {Number(prod.price).toFixed(2).replace('.', ',')}
                        </p>
                        <span className="text-[9px] text-red-500 font-bold uppercase px-1.5 py-0.5 bg-red-500/10 rounded">
                          {prod.plan_type === 'anual' ? 'Anual' : prod.plan_type === 'mensal' ? 'Mensal' : 'Vitalício'}
            ...
            <div className="p-4 bg-zinc-900/50 border border-zinc-800 rounded-2xl border-dashed">
              <div className="flex items-center gap-3 text-zinc-400 text-sm">
                ...
                <p className="font-bold uppercase text-[10px]">Acesso imediato enviado para seu e-mail após confirmação.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-black/80 backdrop-blur-xl border-t border-zinc-800 z-50">
        <div className="max-w-5xl mx-auto">
          <Button 
            ...
            disabled={loading}
            className="w-full h-16 bg-yellow-400 hover:bg-yellow-500 text-black font-black text-xl rounded-2xl shadow-2xl shadow-yellow-400/20 group transition-all italic uppercase"
          >
            {loading ? (
              <Loader2 className="w-7 h-7 animate-spin" />
            ) : (
              <>
                COMPRAR AGORA: R$ {totalAmount.toFixed(2).replace('.', ',')}
...
export default AudiobooksPreCheckout;