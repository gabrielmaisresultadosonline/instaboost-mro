...
const LiveAdmin = () => {
  ...
  const login = async () => {
    setLoggingIn(true);
    try {
      const { data } = await supabase.functions.invoke("livve-admin", {
  ...
  const updateStatus = async (id: string, action: "pauseLive" | "resumeLive" | "endLive") => {
    await supabase.functions.invoke("livve-admin", { body: { action, id } });
  ...
  const saveSettings = async () => {
    await supabase.functions.invoke("livve-admin", {
