...
import { Sparkles, CheckCircle2, ArrowRight, Shield, Clock, Play, Heart, Eye, UserPlus, MessageCircle, Users, Zap, X, Star, Target, Brain, Gift, Monitor, Laptop, Mail, User, CreditCard, Loader2, Phone, Timer, AlertTriangle, TrendingUp, Rocket, Flame, ChevronLeft, ChevronRight } from "lucide-react";
...
const RendaExtraDescontoDuplicate = () => {
        ...
        (async () => {
            try {
                ...
                if (cfg?.whatsapp_number)
                    setWaNumber(cfg.whatsapp_number);
            }
            catch { }
        })();
    }, []);
    ...
    if (!accessGranted) {
        return (<div className="min-h-screen bg-black text-white flex items-center justify-center px-4 py-10">
        <div className="w-full max-w-md bg-gradient-to-br from-zinc-900 to-zinc-950 border border-yellow-500/30 rounded-2xl p-6 sm:p-8 shadow-2xl">
          {gateExpired ? (<>
              ...
              <Button onClick={openWhatsAppExpired} className="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-6 text-base rounded-xl">
              ...
              <form onSubmit={handleGateSubmit} className="space-y-3">
                <Input type="email" value={gateEmail} onChange={(e) => setGateEmail(e.target.value)} placeholder="seu@email.com" className="bg-zinc-800 border-zinc-700 text-white h-12 text-base" required/>
                {gateNotFound && (<p className="text-red-400 text-xs">
                    Email não encontrado. Faça seu cadastro novamente no botão abaixo.
                  </p>)}
                <Button type="submit" disabled={gateLoading} className="w-full bg-gradient-to-r from-yellow-400 to-orange-500 hover:opacity-90 text-black font-bold py-6 text-base rounded-xl">
              ...
              <div className="mt-4 pt-4 border-t border-zinc-800">
                ...
                <Button type="button" onClick={() => setShowRegisterConfirm(true)} variant="outline" className="w-full border-zinc-700 bg-zinc-800/50 text-white hover:bg-zinc-800 font-bold py-5 text-sm rounded-xl">
                  Faça o cadastro
                </Button>
        ...
        {showRegisterConfirm && (<div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={() => setShowRegisterConfirm(false)}>
            <div className="bg-zinc-900 border border-yellow-500/30 rounded-2xl w-full max-w-sm p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex flex-col gap-3">
                <Button onClick={() => { setShowRegisterConfirm(false); setShowRegisterForm(true); }} className="w-full bg-gradient-to-r from-yellow-400 to-orange-500 hover:opacity-90 text-black font-bold py-5 rounded-xl">
                  CADASTRAR E RECEBER A AULA
                </Button>
                <Button variant="outline" onClick={() => setShowRegisterConfirm(false)} className="w-full border-zinc-700 bg-zinc-800/50 text-white hover:bg-zinc-800 font-semibold py-4 rounded-xl">
                  OK, FECHAR
                </Button>
              </div>
            </div>
        ...
        {showRegisterForm && (<div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={() => !regLoading && setShowRegisterForm(false)}>
            <div className="bg-zinc-900 border border-yellow-500/30 rounded-2xl w-full max-w-sm p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
              ...
              <form onSubmit={handleRegisterSubmit} className="space-y-3">
                <Input type="text" value={regNome} onChange={(e) => setRegNome(e.target.value)} placeholder="Nome completo" className="bg-zinc-800 border-zinc-700 text-white h-12" required maxLength={120}/>
                <Input type="tel" value={regWhats} onChange={(e) => setRegWhats(e.target.value)} placeholder="WhatsApp (com DDD)" className="bg-zinc-800 border-zinc-700 text-white h-12" required maxLength={20}/>
                <Input type="email" value={regEmail} onChange={(e) => setRegEmail(e.target.value)} placeholder="seu@email.com" className="bg-zinc-800 border-zinc-700 text-white h-12" required maxLength={255}/>
                <Button type="submit" disabled={regLoading} className="w-full bg-gradient-to-r from-yellow-400 to-orange-500 hover:opacity-90 text-black font-bold py-5 rounded-xl">
                  {regLoading ? <Loader2 className="w-5 h-5 animate-spin"/> : "CADASTRAR E LIBERAR"}
                </Button>
                <button type="button" onClick={() => setShowRegisterForm(false)} className="w-full text-zinc-500 hover:text-zinc-300 text-xs mt-1" disabled={regLoading}>
                  Cancelar
                </button>
              </form>
            </div>
    ...
    return (<div className="min-h-screen bg-black text-white overflow-x-hidden">
      ...
      {showDiscountEndedPopup && (<div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-md p-4">
          <div className="bg-gradient-to-b from-gray-900 to-gray-950 border-2 border-red-500 rounded-2xl p-6 sm:p-8 max-w-md w-full text-center relative animate-in zoom-in-95 duration-300 shadow-[0_0_50px_rgba(239,68,68,0.3)]">
            ...
            {!isDiscountActive ? (<div className="mt-6 p-3 bg-red-950/30 border border-red-900/50 rounded-lg">
                <p className="text-xs text-red-400">
                  Esta oferta foi encerrada temporariamente pela administração.
                </p>
              </div>) : (<button onClick={() => setShowDiscountEndedPopup(false)} className="mt-4 text-gray-400 hover:text-white text-sm underline">
      ...
      <section className="relative pt-16 sm:pt-20 md:pt-24 pb-10 sm:pb-16 px-3 sm:px-4">
        <div className="max-w-5xl mx-auto text-center">
          ...
          <div className="relative mt-8 sm:mt-10 flex items-center justify-center gap-2 sm:gap-4">
            ...
            <Button onClick={scrollToPricing} className="btn-pulse-color text-black font-bold text-sm sm:text-lg px-6 sm:px-10 py-5 sm:py-6 rounded-full shadow-lg shadow-yellow-500/30">
      ...
      <section className="py-16 sm:py-20 px-3 sm:px-4 bg-gradient-to-b from-gray-950 to-black">
        ...
        ].map((item, i) => (<div key={i} className={`bg-gray-900/80 border-2 ${item.border} rounded-2xl p-6 sm:p-8 text-center`}>
        ...
        ].map((item, i) => (<div key={i} className="bg-black/40 border border-amber-500/20 rounded-xl p-4 sm:p-5 text-center">
      ...
      <section className="py-16 sm:py-20 px-3 sm:px-4 bg-gradient-to-b from-gray-950 to-black">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="relative">
            ...
            <div id="feedback-carousel" className="flex gap-3 sm:gap-4 overflow-x-auto pb-4 snap-x snap-mandatory cursor-grab active:cursor-grabbing px-1" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none', WebkitOverflowScrolling: 'touch' }} onMouseDown={(e) => {
        ...
        ].map((video, i) => (<div key={i} className="flex-shrink-0 w-[180px] sm:w-[220px] md:w-[240px] snap-start cursor-pointer group select-none" onClick={(e) => {
                const el = document.getElementById('feedback-carousel');
                if (el?.dataset.dragging === 'true')
                    return;
                openVideo(video.id);
                  ...
                  <div className="relative aspect-[9/16] rounded-xl sm:rounded-2xl overflow-hidden border-2 border-green-500/30 group-hover:border-green-400 transition-all shadow-lg group-hover:shadow-green-500/20">
                    <img src={`https://img.youtube.com/vi/${video.id}/0.jpg`} alt={video.title} className="w-full h-full object-cover pointer-events-none"/>
      ...
      <section className="py-10 sm:py-12 px-3 sm:px-4 bg-gradient-to-b from-black to-gray-950">
        ...
        ].map((item, i) => (<div key={i} className="flex items-start gap-3 bg-gray-900/60 border border-gray-800 rounded-xl p-3 sm:p-4">
      ...
      <section ref={pricingRef} className="py-10 sm:py-16 px-3 sm:px-4 bg-gradient-to-b from-gray-950 to-black">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-gradient-to-b from-gray-900 to-gray-950 border-2 border-green-500 rounded-2xl sm:rounded-3xl p-5 sm:p-8 relative overflow-hidden">
            ...
            <div className="space-y-3 sm:space-y-4 mb-6 sm:mb-8">
              {planFeatures.map((feature, i) => (<div key={i} className="flex items-start gap-3">
        ...
        }} disabled={promoTimeLeft.expired} className="w-full btn-pulse-color text-black font-bold text-base sm:text-xl py-5 sm:py-7 rounded-xl shadow-lg shadow-yellow-500/30 disabled:opacity-50">
      ...
      <section className="py-10 sm:py-16 px-3 sm:px-4 bg-gradient-to-b from-gray-950 to-black">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <Button onClick={() => {
            ...
            navigate("/renddx/promo");
        }} disabled={promoTimeLeft.expired} className="btn-pulse-color text-black font-bold text-sm sm:text-xl px-6 sm:px-12 py-5 sm:py-7 rounded-full shadow-lg shadow-yellow-500/30 disabled:opacity-50">
      ...
      {/* Video Modal */}
      {showVideoModal && (<div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-2 sm:p-4" onClick={() => setShowVideoModal(false)}>
          <button className="absolute top-2 right-2 sm:top-4 sm:right-4 text-white hover:text-gray-300 z-10" onClick={() => setShowVideoModal(false)}>
            <X className="w-6 h-6 sm:w-8 sm:h-8"/>
          </button>
          <div className="w-full max-w-sm aspect-[9/16] max-h-[85vh]" onClick={e => e.stopPropagation()}>
            <iframe src={`https://www.youtube.com/embed/${currentVideoUrl}?autoplay=1&rel=0`} className="w-full h-full rounded-xl" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen/>
          </div>
        </div>)}

      {/* Checkout Modal */}
      {showCheckoutModal && (<div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
          <div className="bg-gray-900 border border-green-500/30 rounded-xl sm:rounded-2xl max-w-md w-full p-4 sm:p-6 relative my-4">
            <button onClick={() => setShowCheckoutModal(false)} className="absolute top-3 right-3 sm:top-4 sm:right-4 text-gray-400 hover:text-white">
            ...
            <form onSubmit={handleCheckout} className="space-y-3 sm:space-y-4">
              <div>
                ...
                <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="seu@email.com" className="bg-gray-800 border-gray-700 text-white text-sm sm:text-base" required/>
              </div>
              ...
              <div>
                ...
                <Input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="(11) 99999-9999" className="bg-gray-800 border-gray-700 text-white text-sm sm:text-base" required/>
              </div>
              ...
              <div>
                ...
                <Input type="text" value={username} onChange={(e) => validateUsername(e.target.value)} placeholder="seunome" className={`bg-gray-800 border-gray-700 text-white text-sm sm:text-base ${usernameError ? 'border-red-500' : ''}`} required/>
                {usernameError && (<p className="text-red-400 text-[10px] sm:text-xs mt-1">{usernameError}</p>)}
                <p className="text-gray-500 text-[10px] sm:text-xs mt-1">
                  Apenas letras minúsculas, sem espaços ou números
                </p>
              </div>
              
              <Button type="submit" disabled={loading || promoTimeLeft.expired} className="w-full bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-bold py-5 sm:py-6 rounded-xl text-sm sm:text-base">
                {loading ? (<>
