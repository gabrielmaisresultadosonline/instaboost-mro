...
import { CheckCircle2, Shield, Loader2, Bot, MessageCircle, Monitor, Crown, Sparkles, Lock, Zap, Users, Plus, } from "lucide-react";
...
const PagamentoTesvc = () => {
    ...
    const totalPrice = BASE_PLAN.price +
    ...
    const handleCheckout = async (e) => {
        ...
        if (!phone || phone.replace(/\D/g, "").length < 10) {
            toast.error("WhatsApp inválido com DDD");
            return;
        }
        if (!username || username.length < 4) {
            toast.error("Usuário deve ter no mínimo 4 caracteres");
            return;
        }
        if (usernameError) {
        ...
        try {
            const extras = [];
            if (withSupport)
                extras.push("suporte-whatsapp");
            if (withExtraAccounts)
                extras.push("mais-3-contas");
            if (withRendaExtra)
                extras.push("renda-extra");
    ...
    return (<div className="min-h-screen bg-white text-zinc-900 selection:bg-amber-300/60">
          ...
          <form onSubmit={handleCheckout} className="lg:col-span-3 space-y-5">
            ...
            <div className="bg-gradient-to-br from-white to-zinc-50 border-2 border-amber-400 rounded-2xl p-6 space-y-4 shadow-2xl relative overflow-hidden group/bumps">
              ...
              <div className="grid gap-4">
                ...
                <button type="button" onClick={() => setWithRendaExtra((v) => !v)} className={`group relative w-full text-left p-5 rounded-2xl border-b-4 transition-all duration-300 transform hover:-translate-y-1 active:translate-y-0 flex items-start gap-4 ${withRendaExtra
            ...
            : "border-amber-200 bg-white hover:border-amber-400 hover:shadow-md"}`}>
                  <div className={`mt-0.5 w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${withRendaExtra ? "bg-amber-500 border-amber-500 text-white" : "border-amber-300 group-hover:border-amber-400"}`}>
                  ...
                  {!withRendaExtra && (<div className="absolute -left-12 top-1/2 -translate-y-1/2 hidden md:block">
                ...
                <button type="button" onClick={() => setWithSupport((v) => !v)} className={`group relative w-full text-left p-5 rounded-2xl border-b-4 transition-all duration-300 transform hover:-translate-y-1 active:translate-y-0 flex items-start gap-4 ${withSupport
            ...
            : "border-zinc-300 bg-white hover:border-emerald-400 hover:shadow-md"}`}>
                  <div className={`mt-0.5 w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${withSupport ? "bg-emerald-500 border-emerald-500 text-white" : "border-zinc-300 group-hover:border-emerald-400"}`}>
                ...
                <button type="button" onClick={() => setWithExtraAccounts((v) => !v)} className={`group relative w-full text-left p-5 rounded-2xl border-b-4 transition-all duration-300 transform hover:-translate-y-1 active:translate-y-0 flex items-start gap-4 ${withExtraAccounts
            ...
            : "border-zinc-300 bg-white hover:border-indigo-400 hover:shadow-md"}`}>
                  <div className={`mt-0.5 w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${withExtraAccounts ? "bg-indigo-500 border-indigo-500 text-white" : "border-zinc-300 group-hover:border-indigo-400"}`}>
            ...
            <div className="bg-white border border-zinc-200 rounded-2xl p-6 space-y-4 shadow-sm">
              ...
              <div>
                ...
                <Input value={username} onChange={(e) => validateUsername(e.target.value)} placeholder="apenas letras minúsculas" className="mt-1 bg-white border-zinc-300"/>
                {usernameError && <p className="text-xs text-red-500 mt-1 font-medium">{usernameError}</p>}
                {usernameAvailable === true && !usernameError && (<p className="text-xs text-emerald-600 mt-1 font-medium">✓ Usuário disponível</p>)}
                {checkingUsername && <p className="text-xs text-zinc-500 mt-1">Verificando…</p>}
              </div>

              <Button type="submit" disabled={loading} className="w-full h-auto min-h-14 py-3 px-4 whitespace-normal text-center text-sm sm:text-base font-black bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white shadow-lg shadow-emerald-500/30 transition-all">
                {loading ? (<><Loader2 className="w-4 h-4 mr-2 animate-spin"/> Gerando link…</>) : (<>🚀 Pagar agora · {formatBRL(totalPrice)}</>)}
              </Button>
            ...
            <div className="sticky top-24 bg-white border border-zinc-200 rounded-2xl p-6 space-y-4 shadow-lg">
              ...
              <div className="rounded-xl bg-gradient-to-br from-emerald-50 to-white border border-emerald-200 p-4">
                ...
                <div className="mt-4 space-y-1.5 text-sm border-t border-emerald-200 pt-3">
                  ...
                  {withSupport && (<div className="flex items-center justify-between text-emerald-700">
                  ...
                  {withRendaExtra && (<div className="flex items-center justify-between text-amber-700">
                  ...
                  {withExtraAccounts && (<div className="flex items-center justify-between text-amber-700">
              ...
              <ul className="space-y-2.5">
                {BENEFITS.map((b, i) => (<li key={i} className="flex items-start gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5"/>
                    <span className="text-sm text-zinc-700">{b.text}</span>
                  </li>))}
                {withSupport && (<li className="flex items-start gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5"/>
