...
import {
  Radio, Play, Pause, StopCircle, Plus, BarChart3, Settings, Upload, Eye, Users, Percent, LogOut
} from "lucide-react";
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="bg-gray-900 border-b border-gray-800 px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button
            variant={tab === "lives" ? "default" : "ghost"}
            size="sm"
          ...
          <Button
            ...
            size="sm"
            onClick={() => {
              sessionStorage.removeItem("live_admin_auth");
          ...
          <>
            ...
            {!creating ? (
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
                ...
                <div>
                  <label className="text-sm text-gray-400 mb-1 block">Vídeo da Live</label>
                  {newLive.video_url ? (
                    ...
                    <label className="flex items-center gap-2 cursor-pointer bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 hover:border-red-500 transition">
                      <Upload className="w-5 h-5 text-gray-400" />
                      <span className="text-gray-400 text-sm">Clique para enviar o vídeo (MP4)</span>
                      <input type="file" accept="video/*" className="hidden" onChange={(e) => handleVideoUpload(e)} />
                    </label>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-sm text-gray-400">Viewers Mín</label>
                    <Input
                      type="number"
                      value={newLive.fake_viewers_min}
                      onChange={(e) => setNewLive((p) => ({ ...p, fake_viewers_min: Number(e.target.value) }))}
                      className="bg-gray-800 border-gray-700 text-white"
                    />
                  </div>
                  <div>
                    <label className="text-sm text-gray-400">Viewers Máx</label>
                    <Input
                      type="number"
                      value={newLive.fake_viewers_max}
                      onChange={(e) => setNewLive((p) => ({ ...p, fake_viewers_max: Number(e.target.value) }))}
                      className="bg-gray-800 border-gray-700 text-white"
                    />
                  </div>
                </div>
                ...
                <Input
                  ...
                  value={newLive.whatsapp_group_link}
                  onChange={(e) => setNewLive((p) => ({ ...p, whatsapp_group_link: e.target.value }))}
                  className="bg-gray-800 border-gray-700 text-white"
                />
                <Input
                  placeholder="Título do CTA final"
                  value={newLive.cta_title}
                  onChange={(e) => setNewLive((p) => ({ ...p, cta_title: e.target.value }))}
                  className="bg-gray-800 border-gray-700 text-white"
                />
                <Textarea
                  placeholder="Descrição do CTA final"
                  value={newLive.cta_description}
                  onChange={(e) => setNewLive((p) => ({ ...p, cta_description: e.target.value }))}
                  className="bg-gray-800 border-gray-700 text-white"
                />
                <Input
                  placeholder="Texto do botão CTA"
                  value={newLive.cta_button_text}
                  onChange={(e) => setNewLive((p) => ({ ...p, cta_button_text: e.target.value }))}
                  className="bg-gray-800 border-gray-700 text-white"
                />
                <Input
                  placeholder="Link do botão CTA (grupo desconto)"
                  value={newLive.cta_button_link}
                  onChange={(e) => setNewLive((p) => ({ ...p, cta_button_link: e.target.value }))}
                  className="bg-gray-800 border-gray-700 text-white"
                />
                  ...
                  <div className="flex items-start justify-between gap-4 flex-wrap">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        Viewers: {s.fake_viewers_min} - {s.fake_viewers_max}
                      </p>
                    </div>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      <label className="cursor-pointer">
                        <Button size="sm" variant="outline" asChild className="gap-1">
                          <span>
                            <Upload className="w-3 h-3" /> Vídeo
                          </span>
                        </Button>
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-800">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        ...
                        <div className="bg-gray-800 rounded-lg p-3 text-center">
                          ...
                          <p className="text-xs text-gray-400">50-95% assistido</p>
                        </div>
                        <div className="bg-gray-800 rounded-lg p-3 text-center">
                          <Percent className="w-5 h-5 mx-auto mb-1 text-red-400" />
                          <p className="text-2xl font-bold text-red-400">{analyticsData.watchedLess50}</p>
                          <p className="text-xs text-gray-400">&lt; 50% assistido</p>
                        </div>
                      </div>
                    </div>
                  )}
