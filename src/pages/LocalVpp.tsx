...
const LocalVPP = () => {
  ...
  const [aulaData, setAulaData] = useState<string>("20/07");
  ...
  useEffect(() => {
    document.title = "LocalVPP | Como a MRO ajuda deliveries a atrair mais clientes";
    (async () => {
      ...
      try {
        const { data } = await supabase.functions.invoke("localvpp-admin", { body: { action: "get_public_settings" } });
        if (data?.settings) {
          setPreco(Number(data.settings.preco) || 10);
          setAulaData(data.settings.aula_data || "20/07");
  ...
  const scrollToPreco = () => {
    const fbq = (window as any).fbq;
    if (fbq) fbq("track", "ViewContent", { content_name: "LocalVPP MRO" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("localvpp-checkout", {
      ...
      if (fbq) fbq("track", "InitiateCheckout",
        { value: preco, currency: "BRL", content_name: "LocalVPP MRO" },
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pt-6 md:pt-12">
        ...
        <div className="max-w-3xl mx-auto text-center">
          <div className="space-y-7 flex flex-col items-center">
            ...
            <p className="text-base md:text-xl text-neutral-200 font-medium -mt-1">
              Delivery, aumente seus clientes{" "}
              <span className="relative inline-block">
                <span className="relative z-10 bg-gradient-to-r from-yellow-300 via-yellow-400 to-yellow-500 bg-clip-text text-transparent font-bold">sem precisar investir em anúncios pagos!</span>
                <span className="absolute inset-x-0 bottom-1 h-2 bg-yellow-400/20 blur-md -z-0" />
              </span>{" "}
              Aprenda como a MRO pode te ajudar.
            </p>
