...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "track_visit") {
      await supabase.from("delivery_visits").insert({
    ...
    if (action === "check_paid") {
      const { data } = await supabase.from("delivery_orders").select("status,email,amount").eq("nsu_order", body.nsu).maybeSingle();
    ...
    if (action === "get_settings") {
      const { data } = await supabase.from("delivery_settings").select("*").limit(1).maybeSingle();
    ...
    if (action === "save_settings") {
      ...
      const { data: existing } = await supabase.from("delivery_settings").select("id").limit(1).maybeSingle();
    ...
    if (action === "list_orders") {
      const { data } = await supabase.from("delivery_orders").select("*").order("created_at", { ascending: false }).limit(500);
      return json({ success: true, orders: data || [] });
    }

    if (action === "list_visits") {
      const { data } = await supabase.from("delivery_visits").select("*").order("created_at", { ascending: false }).limit(200);
      const { count } = await supabase.from("delivery_visits").select("*", { count: "exact", head: true });
