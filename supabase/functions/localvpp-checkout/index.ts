...
const log = (s: string, d?: unknown) =>
  console.log(`[LOCALVPP-CHECKOUT] ${s}`, d ? JSON.stringify(d) : "");

const genNSU = () =>
  `LOCALVPP${Date.now().toString(36)}${Math.random().toString(36).substring(2, 8)}`.toUpperCase();
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: settings } = await supabase.from("localvpp_settings").select("preco").limit(1).maybeSingle();
    const amount = Number(settings?.preco) || 19;
    const priceCents = Math.round(amount * 100);
    const nsu = genNSU();

    const redirectUrl = `https://maisresultadosonline.com.br/localvpp/obrigado?nsu=${nsu}`;
    const webhookUrl = `${Deno.env.get("SUPABASE_URL")}/functions/v1/infinitepay-webhook`;
    const description = `LOCALVPP_${cleanEmail}`;
