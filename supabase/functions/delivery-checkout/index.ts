...
const log = (s: string, d?: unknown) =>
  console.log(`[DELIVERY-CHECKOUT] ${s}`, d ? JSON.stringify(d) : "");

const genNSU = () =>
  `DELIVERY${Date.now().toString(36)}${Math.random().toString(36).substring(2, 8)}`.toUpperCase();
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: settings } = await supabase.from("delivery_settings").select("preco").limit(1).maybeSingle();
    const amount = Number(settings?.preco) || 19;
    const priceCents = Math.round(amount * 100);
    const nsu = genNSU();

    const redirectUrl = `https://maisresultadosonline.com.br/delivery/obrigado?nsu=${nsu}`;
    const webhookUrl = `${Deno.env.get("SUPABASE_URL")}/functions/v1/infinitepay-webhook`;
    const description = `DELIVERY_${cleanEmail}`;
