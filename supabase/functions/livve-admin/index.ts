  ...
  try {
    ...
    if (action === "trackAnalytics") {
      ...
      if (existing) {
        ...
        await supabase.from("livve_analytics").insert({
          session_id,
          visitor_id,
          watch_percentage,
          device_type,
    ...
    if (action === "updateSettings") {
      ...
      const { data: existing } = await supabase.from("livve_settings").select("id").limit(1).single();
    ...
    if (action === "getSettings") {
      const { data } = await supabase.from("livve_settings").select("*").limit(1).single();
