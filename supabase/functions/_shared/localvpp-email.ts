...
export const buildLocalVppEmail = (name: string, groupLink: string, aulaData: string) => `<!DOCTYPE html>
...
<tr><td align="center">
  <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#111827;border-radius:16px;overflow:hidden;border:1px solid #1f2937;">
    <tr><td style="background:linear-gradient(135deg,#f97316 0%,#ea580c 100%);padding:32px 24px;text-align:center;">
      ...
      <p style="color:#ffedd5;margin:0;font-size:15px;">LocalVPP — aumente seus clientes sem investir em anúncios</p>
    </td></tr>
...
export const sendLocalVppEmail = async (
  ...
  const html = buildLocalVppEmail(name || "cliente", groupLink || "#", aulaData || "em breve");
  ...
  try {
    ...
    await client.send({
      from: "LocalVPP MRO <suporte@maisresultadosonline.com.br>",
      to,
      subject,
