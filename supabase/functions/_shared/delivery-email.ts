...
export const buildDeliveryEmail = (name: string, groupLink: string, aulaData: string) => `<!DOCTYPE html>
...
<tr><td align="center">
  <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#111827;border-radius:16px;overflow:hidden;border:1px solid #1f2937;">
    <tr><td style="background:linear-gradient(135deg,#f97316 0%,#ea580c 100%);padding:32px 24px;text-align:center;">
      ...
      <p style="color:#ffedd5;margin:0;font-size:15px;">Delivery — aumente seus clientes sem investir em anúncios</p>
    </td></tr>
    <tr><td style="padding:32px 28px;color:#e5e7eb;">
      ...
      <p style="font-size:15px;line-height:1.6;color:#d1d5db;margin:0 0 20px;">
        Sua vaga está confirmada! No dia <strong style="color:#fb923c;">${aulaData}</strong> vamos te mostrar, passo a passo, como a <strong style="color:#fff;">MRO</strong> ajuda deliveries a atrair mais clientes <strong>sem precisar investir em anúncios</strong>.
      </p>
      <div style="background:#0f172a;border-left:4px solid #fb923c;padding:18px 20px;border-radius:10px;margin:20px 0;">
      ...
      <p style="font-size:14px;color:#9ca3af;line-height:1.6;margin:20px 0 0;">
        Prepare-se: você vai descobrir uma estratégia simples e prática para encher os pedidos do seu delivery com clientes de verdade.
      </p>
...
export const sendDeliveryEmail = async (
  ...
  if (!smtpPassword) {
    console.error("[DELIVERY-EMAIL] SMTP_PASSWORD not set");
    return false;
  }

  const subject = sanitizeEmailSubject("🎉 Acesso confirmado — Entre no grupo e aprenda a atrair mais clientes pro seu delivery");
  const html = buildDeliveryEmail(name || "cliente", groupLink || "#", aulaData || "em breve");
  ...
  try {
    ...
    await client.send({
      from: "Delivery MRO <suporte@maisresultadosonline.com.br>",
      to,
      subject,
    ...
    if (supabase) {
      await supabase.from("delivery_email_logs").insert({
    ...
    console.error("[DELIVERY-EMAIL] Error:", errorMsg);
    if (supabase) {
      await supabase.from("delivery_email_logs").insert({
