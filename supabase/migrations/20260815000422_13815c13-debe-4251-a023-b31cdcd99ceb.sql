...
-- 2. Create lotargrupos_users table
...
GRANT SELECT ON public.lotargrupos_users TO anon;
GRANT ALL ON public.lotargrupos_users TO service_role;

GRANT SELECT ON public.lotargrupos_lessons TO authenticated;
GRANT SELECT ON public.lotargrupos_lessons TO anon;
GRANT ALL ON public.lotargrupos_lessons TO service_role;

-- 6. Policies
CREATE POLICY "Anon can check active emails" ON public.lotargrupos_users
    FOR SELECT TO anon
    USING (status = 'active');
...
CREATE POLICY "Anon can view active lessons" ON public.lotargrupos_lessons
    FOR SELECT TO anon
    USING (status = 'active');
